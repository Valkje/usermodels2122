...
class BufferAction {
	 ...
	 * 
	 * @param slot the slot
	 * @return the slot action for that slot, or <tt>null</tt> if the slot is not
	 *         present
	 */
	 ...
	 * 
	 * @param slot the slot
	 * @returns <tt>true</tt> if the buffer action specifies the slot in any slot
	 *          action, and <tt>false</tt> otherwise
	 */
	 ...
	 * @returns <tt>true</tt> if the buffer action specifies the slot value in any
	...
	/**
	 * Checks whether the buffer action represents the direct setting of a buffer
	 * (as in <tt>=buffer> =chunk</tt>).
	 * 
	...
	void expandDirectAction(Instantiation inst) {
		...
		// when used for production compilation, expansion doesn't always have the chunk
		// in the buffer!
