...
Nevertheless, we interpreted this as suggesting that increases in cognitive load can be detected by monitoring short-term deviations from the long-term trend, which would align well with similar systems that have been created in the past: For example Katidioti et al. (2016), in their interruption management system, waited until the last 50 raw pupil samples (base-lined) dropped below a long-term threshold (again based on a moving average) before scheduling an interruption. Thus, they also treated this short-term deviation from the long-term baseline as an indicator of a change (in their case a decrease) in cognitive load (Katidioti et al., 2016). An advantage of relying on a moving average for short-term trend estimation as well, as was done by Mindakis and Lohan (2018), is that this will automatically reduce the impact possible outliers in the recorded raw pupil size measures might have on a decision system. Therefore, we wanted to integrate these two moving averages in our system as well. However, relying on a singular maximum value for the decision threshold, as was probably done by Mindakis and Lohan (2018), is unlikely to be robust to the dynamic changes of the environment (e.g. light pollution) when driving a car.

Therefore, we wanted to change the decision mechanism to utilize more information. Specifically, we wanted our system to account for the normal "dispersion" (see: [here](https://en.wikipedia.org/wiki/Statistical_dispersion) for an overview) of the short-term trend's fluctuations around the long-term pupil size average. We can then utilize a measure of this variation to decide when the short-term changes in pupil size deviate too extremely from the long-term changes in pupil size, which is a common strategy in "outlier detection systems" (see for example Mehrang, 2015 for an approach utilizing the median absolute deviation MAD and see [here](https://scikit-learn.org/stable/modules/outlier_detection.html) for a definition of outlier detection). Since we wanted to work with averages we opted to compute a cumulative estimate of the standard deviation (essentially the root mean square deviation) of the average changes in pupil size during the last "window_size_short_term" samples (short-term pupil trend) from the average changes in pupil size during the last "window_size_long_term" samples (long-term pupil trend) instead of the MAD (see: [here](https://en.wikipedia.org/wiki/Standard_deviation), [here](https://en.wikipedia.org/wiki/Root-mean-square_deviation) and the Python implementation for details). 

The algorithm below details how we combine the standard deviation and the moving averages to reach an automation decision (see AdaptiveAutomationSystem.java and main.py files):
